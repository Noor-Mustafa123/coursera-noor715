...
/**
 * The Class TransferStagingServiceImpl.
 */
@Log4j2
public class TransferStagingServiceImpl extends HttpServlet {
    ...
    public void doPost( HttpServletRequest request, HttpServletResponse response ) {
        ...
        try ( PrintWriter printWriter = response.getWriter() ) {
            ...
            ExceptionLogger.logException( e, TransferStagingServiceImpl.class );
        }
                    ...
                    try ( InputStream stream = item.openStream() ) {
                        if ( !item.isFormField() ) {
                            if ( item.getContentType() == null ) {
                                fail( printWriter,
                                        MessageBundleFactory.getMessage( Messages.NO_CONTENT_TYPE_PROVIDED_TO_UPLOAD.getKey() ) );
                            }
                            String destPath = request.getHeader( ConstantRequestHeader.DEST_PATH );
                            writeToDiskInStaging( request.getHeader( ConstantRequestHeader.USER_UID ), stream, destPath );
                        }
                    }
                }
            } catch ( Exception e ) {
