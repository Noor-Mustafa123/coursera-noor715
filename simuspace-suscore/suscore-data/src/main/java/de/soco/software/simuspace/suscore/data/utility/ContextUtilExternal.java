...
/**
 * This util keep context in perticular order and add divider on certain place.
 *
 * @author Noman Arshad
 * @since 2.0
 */
public class ContextUtilExternal {

    /**
     * The Constant RESTORE_BULK.
     */
    private static final String RESTORE_BULK = "4100083x4";

    /**
     * The Constant DELETE_BULK2.
     */
    private static final String DELETE_BULK2 = "4100082x4";
     ...
     * @return the list
     */
    public static List< ContextMenuItem > allOrderedContext( boolean hasTranslation, String authToken,
            List< ContextMenuItem > unOrderedContext ) {
        ...
        while ( iter.hasNext() ) {
            ...
            if ( createOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( editOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( deleteOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( manageOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( openOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( runOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( synchOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( permissionAndTypeAndMetaDataOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( exportAndImportAndLinkiOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( restoreOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( downloadCSVOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
                continue;
            }
            if ( generateImageOptionOrderedContext( contextMenuItem, orderedContext, hasTranslation, authToken ) ) {
                iter.remove();
            }
        }
    ...
    private static boolean createOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
            boolean hasTranslation, String authToken ) {
        boolean found = false;
        if ( contextMenuItem.getTitle().equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE ) ) ) {
            orderedContext[ 1 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE_PROJECT ) ) ) {
            orderedContext[ 2 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE_WORKFLOW_PROJECT ) ) ) {
            orderedContext[ 3 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE_WORKFLOW ) ) ) {
            orderedContext[ 4 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE_LOADCASE ) ) ) {
            orderedContext[ 5 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE_DUMMY_VARIANT ) ) ) {
            orderedContext[ 6 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE_COPY ) ) ) {
            orderedContext[ 7 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE_SCHEME ) ) ) {
            orderedContext[ 8 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE_CATEGORY ) ) ) {
            orderedContext[ 9 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CREATE_LOCAL_DIRECTORY ) ) ) {
    ...
    private static boolean editOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
            boolean hasTranslation, String authToken ) {
        boolean found = false;
        if ( contextMenuItem.getTitle().equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, EDIT ) ) ) {
            orderedContext[ 31 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, EDIT_CATAGORIES ) ) ) {
            orderedContext[ 32 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, EDIT_SCHEME ) ) ) {
    ...
    private static boolean openOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
        ...
        if ( contextMenuItem.getTitle().equals( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, OPEN_FILE ) ) ) {
            orderedContext[ 54 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .contains( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, OPEN_WITH ) ) ) {
            ContextMenuItem openWithTitleItem = new ContextMenuItem();
            openWithTitleItem.setTitle( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, OPEN_WITH ) );
            openWithTitleItem.setLinkClass( "disabled" );
            orderedContext[ 54 ] = openWithTitleItem;
            contextMenuItem.setTitle( contextMenuItem.getTitle().replace(
                    MessageBundleFactory.getExternalMessage( hasTranslation, authToken, OPEN_WITH ), ConstantsString.EMPTY_STRING ) );
    ...
    private static boolean deleteOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
            boolean hasTranslation, String authToken ) {
        boolean found = false;
        if ( contextMenuItem.getTitle().equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, DELETE ) ) ) {
            orderedContext[ 41 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, DELETE_BULK ) )
                || contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, DELETE_BULK2 ) ) ) {
    ...
    private static boolean manageOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
            boolean hasTranslation, String authToken ) {
        boolean found = false;
        if ( contextMenuItem.getTitle().equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, MANAGE ) ) ) {
    ...
    private static boolean runOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
            boolean hasTranslation, String authToken ) {
        boolean found = false;
        if ( contextMenuItem.getTitle().equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, RUN ) ) ) {
            orderedContext[ 61 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, RUN_WORKFLOW ) ) ) {
            orderedContext[ 62 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, RUN_SCHEME ) ) ) {
    ...
    private static boolean synchOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
            boolean hasTranslation, String authToken ) {
        boolean found = false;
        if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, DOWNLOAD ) ) ) {
            orderedContext[ 71 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, UPLOAD_SYNCH ) ) ) {
            orderedContext[ 72 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, DOWNLOAD_SYNCH ) ) ) {
            orderedContext[ 73 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, ABORT_SYNCH ) ) ) {
            orderedContext[ 74 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, DISCARD_SYNCH ) ) ) {
            orderedContext[ 75 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, EXPORT_DOWNLOAD_LOCAL_SYNCH ) ) ) {
            orderedContext[ 76 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CHECK_IN_SYNCH ) ) ) {
            orderedContext[ 78 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CHECK_OUT_SYNCH ) ) ) {
            orderedContext[ 79 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CHANGE_TYPE_SYNCH ) ) ) {
            orderedContext[ 80 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CURVE_COMPARE ) ) ) {
            orderedContext[ 81 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, DOWNLOAD_JOB_LOGS ) ) ) {
    ...
    private static boolean permissionAndTypeAndMetaDataOptionOrderedContext( ContextMenuItem contextMenuItem,
            ContextMenuItem[] orderedContext, boolean hasTranslation, String authToken ) {
        boolean found = false;
        if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, ADD_META_DATA ) ) ) {
            orderedContext[ 91 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, CHANGE_STATUS ) ) ) {
            orderedContext[ 92 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, PERMISION ) ) ) {
    ...
    private static boolean exportAndImportAndLinkiOptionOrderedContext( ContextMenuItem contextMenuItem,
        ...
        boolean found = false;
        if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, EXPORT_DATA ) ) ) {
            orderedContext[ 101 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, IMPORT_WORKFLOW ) ) ) {
            orderedContext[ 102 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, MOVE_TO ) ) ) {
            orderedContext[ 103 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, LINK ) ) ) {
            orderedContext[ 104 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, LINK_ITEMS ) ) ) {
            orderedContext[ 105 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, REMOVE_LINK_TITLE ) ) ) {
    ...
    private static boolean restoreOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
        ...
        boolean found = false;
        if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, RESTORE_BULK ) ) ) {
            orderedContext[ 121 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, RESTORE ) ) ) {
    ...
    private static boolean downloadCSVOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
        ...
        boolean found = false;
        if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, DOWNLOAD_CSV_FILE ) ) ) {
            orderedContext[ 131 ] = contextMenuItem;
            found = true;
            orderedContext[ 132 ] = addDivider();
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, "4100049x4" ) ) ) {
            orderedContext[ 133 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, "4100050x4" ) ) ) {
            orderedContext[ 134 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, "4100051x4" ) ) ) {
            orderedContext[ 135 ] = contextMenuItem;
            found = true;
        } else if ( contextMenuItem.getTitle()
                .equalsIgnoreCase( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, "4100052x4" ) ) ) {
    ...
    private static boolean generateImageOptionOrderedContext( ContextMenuItem contextMenuItem, ContextMenuItem[] orderedContext,
            boolean hasTranslation, String authToken ) {
        boolean found = false;
        if ( contextMenuItem.getTitle().contains( MessageBundleFactory.getExternalMessage( hasTranslation, authToken, "4100052x4" ) ) ) {
